/*
    JS 데이터
        ...
        5) 배열(객체) : [데이터, 데이터, 데이터]
            ...
            <삽입 / 삭제>   1. 변수명.push() 2. 변수명.splice()
            <검색>          1. 변수명.indexOf() 2. for(let i=0; i<변수명.length; i++){}
        6) 함수(객체) : function(){}
            ...
            <선언> const 변수명 = function 함수명(매개변수) {실행문; return;}
            <호출> JS - 함수명();       HTML - <마크업 이벤트속성="함수명()">
        ...
        7) 객체 : {속성명 : 데이터, 속성명 : 데이터, 속성명 : 데이터}  
            > 여러 데이터를 속성명과 데이터의 한 쌍으로 이루어진 여러 쌍을 묶은 형식, 속성명
            <선언> let 변수명 = {이름 : `유재석`, 나이 : 40}
            <호출> 1. 변수명 2. 변수명.이름 3. 변수명[`이름`]
            <수정> 1. 변수명.아름 = 강호동
            <속성명 삭제> 1. delete 변수명.속성명
            <속성명 추가> 1. 변수명.새로운속성명 = 데이터

    JS 데이터 1개 저장하는 곳 : let, const

    라이브러리 / 미리 만들어진 함수 또는 객체
        > 내장 라이브러리(별도 설치가 없어도 사용 가능한 함수 또는 객체)
            1. console -> 객체
                log -> 속성    -> console.log()
            2. Math -> 객체
                random -> 속성 -> Math.random()
            3. 배열 -> 객체
                배열.push(), 배열.splice(), 배열.indexOf(), 배열.length
            4. 문자(잠시 객체로 승급)
            ...
            7. * Dom 객체 *
                D(document = 문서 = HTML = 현재 JS가 실행된 HTML)
                O(Object = 객체)
                M(Model = 모형 / 본보기)
