/* 
    - 주간 영화 목록 API
        ...
        4. 해당 페이지에 나온 데이터 / 자료 코드를 복사해서 JS 파일 변수에 대입
            변수 = 복사한 코드
        5. 순위 영화명 개봉일 누적관객수 누적매출액 순으로 html 테이블에 출력
*/
...
console.log(영화API.boxOfficeResult);                                // 자료 결과정보, 주간 박스오피스 전체정보
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList);            // 주간 박스오피스의 영화 정보
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList[0].rank);    // 1번째 영화 정보의 순위
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList[0].movieNm); // 1번째 영화 정보의 영화명

// 2. html 출력 함수 > 실행조건 1) JS 열렸을 때 함수 1번 실행
function movie(){
    let movieRank = document.querySelector(`#movieRank`);
    let html = ``;
    let movieList = 영화API.boxOfficeResult.weeklyBoxOfficeList;
    for(let i=0; i<movieList.length; i++){
        // html에 반복해서 html 문자열 누적 대입 
        html += `
                <tr>
                    <td>
                        ${movieList[i].rank}
